...
// --- ICONOS ---
const IconPlus = () => <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>;
const IconUpdate = () => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>;
const IconFile = () => <svg className="w-8 h-8 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>;
const IconUpload = () => <svg className="w-10 h-10 text-gray-400 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>;
...
const FuentesEstaticas = () => {
  ...
  // Path: /admin/fuentes
  const API_URL = "http://localhost:8080/fuenteEstatica/admin/fuentes";
  ...
  const handleSubmit = async () => {
    ...
    const toastId = toast.loading(modoEdicion ? "Actualizando fuente..." : "Creando fuente...");
    ...
    try {
      ...
      if (response.ok) {
        toast.success(modoEdicion ? "Fuente actualizada con éxito" : "Fuente creada con éxito", { id: toastId });
        setIsModalOpen(false);
        cargarFuentes();
      } else {
        const errorData = await response.json().catch(() => ({}));
        // Si el Gateway falla en pasar el POST, podría devolver 405 Method Not Allowed o 500
        toast.error(`Error: ${errorData.message || "No se pudo procesar la solicitud (Revise Gateway)"}`, { id: toastId });
      }
  ...
  const formatearFecha = (fechaISO) => {
    ...
    return new Date(fechaISO).toLocaleDateString() + " " + new Date(fechaISO).toLocaleTimeString();
  };
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {fuentes.map((fuente) => (
            ...
            >
              <div className={`p-3 rounded-xl ${fuente.activa ? 'bg-green-50 dark:bg-green-900/30 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                <IconFile />
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 truncate" title={fuente.nombreArchivoOriginal}>
            ...
            <div className="space-y-6">
              ...
              {modoEdicion && (
                <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                  <span className="text-xs font-bold text-gray-500 uppercase">Fuente Seleccionada</span>
                  <p className="text-lg font-bold text-gray-800 dark:text-white">{nombreFuente}</p>
                  <p className="text-xs text-gray-400 mt-1">ID: {fuenteAEditarId}</p>
                </div>
              )}
                ...
                {archivoSeleccionado ? (
                  <div className="flex flex-col items-center">
                    <div className="w-12 h-12 bg-green-100 dark:bg-green-800 rounded-full flex items-center justify-center mb-2">
                       <svg className="w-6 h-6 text-green-600 dark:text-green-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    </div>
                    <p className="font-bold text-green-700 dark:text-green-400 break-all">
                      {archivoSeleccionado.name}
                    </p>
                    <p className="text-xs text-green-600/70 mt-1">Click para cambiar</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center">
                    <IconUpload />
