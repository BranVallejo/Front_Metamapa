...
const IconServer = () => <svg className="w-8 h-8 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01" /></svg>;
const IconLink = () => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>;
...
const FuentesDemo = () => {
  ...
  // CONFIGURACIÓN API (Gateway Puerto 8080)
  // Modulo: fuenteDemo -> http://localhost:8080/fuenteDemo/admin/fuentes
  const API_URL = "http://localhost:8080/fuenteDemo/admin/fuentes";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ nombreFuente: "", url: "", pathApi: "", email: "", password: "", requiereAuth: false }); // Reset
        cargarFuentes();
      } else if (response.status === 401) {
        // El controller devuelve 401 si falla la conexión externa
        toast.error("Error de autenticación con la fuente externa. Verificá las credenciales.", { id: toastId });
      } else {
          ...
          {fuentes.map((fuente) => (
            ...
            >
              <div className="flex items-start gap-4 mb-3">
                <div className={`p-3 rounded-xl ${fuente.activa ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'bg-gray-100'}`}>
                  <IconServer />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
                        fuente.activa ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                    }`}>
                  ...
                  {(fuente.nombreDetectado || fuente.etiquetaDetectada) && (
                    <div className="mt-1 flex gap-2 text-xs text-indigo-600 dark:text-indigo-400">
                      {fuente.nombreDetectado && <span>Detectado: {fuente.nombreDetectado}</span>}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700/50 text-xs font-mono text-gray-600 dark:text-gray-300 overflow-hidden">
                <div className="flex items-center gap-2 mb-1 truncate">
                  <IconLink />
                  <span className="truncate" title={fuente.urlBase}>{fuente.urlBase}</span>
                </div>
                <div className="pl-6 text-gray-500 truncate" title={fuente.pathApi}>
                  Path: {fuente.pathApi}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-8 border border-gray-100 dark:border-gray-800 animate-scaleUp">
            ...
            <div className="mb-6 text-center">
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Conectar Nueva API</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Nombre Identificador</label>
                <input 
                  type="text" name="nombreFuente"
                  ...
                  value={formData.nombreFuente} onChange={handleInputChange}
                />
              </div>

              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">URL Base</label>
                  <input 
                    type="text" name="url"
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Path API</label>
                  <input 
                    type="text" name="pathApi"
                    className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-sm"
                    placeholder="/v1/data"
                    value={formData.pathApi} onChange={handleInputChange}
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                <input 
                  type="checkbox" id="requiereAuth" name="requiereAuth"
                  checked={formData.requiereAuth} onChange={handleInputChange}
                  className="w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500 border-gray-300"
                />
                <label htmlFor="requiereAuth" className="text-sm text-gray-700 dark:text-gray-300 font-medium select-none cursor-pointer">
