...
const GestionColecciones = () => {
  ...
  const cargarColecciones = async () => {
    setCargando(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_URL_INICIAL_GESTOR}/publica/colecciones`);
  ...
  const transformarDatosParaBackend = (datosFormulario) => {
    ...
    if (Array.isArray(datosFormulario.criterios.categoria)) {
      datosFormulario.criterios.categoria.forEach((cat) => criterios.push({ tipo: "porcategoria", params: { categoriaDeseada: cat } }));
    }
    if (datosFormulario.criterios.titulo?.trim()) criterios.push({ tipo: "portitulo", params: { tituloBuscado: datosFormulario.criterios.titulo } });
    if (datosFormulario.criterios.descripcion?.trim()) criterios.push({ tipo: "pordescripcion", params: { fraseClave: datosFormulario.criterios.descripcion } });
    if (datosFormulario.criterios.contienemultimedia !== "") criterios.push({ tipo: "contienemultimedia", params: { multimedia: datosFormulario.criterios.contienemultimedia } });
    if (datosFormulario.criterios.porfechacargadesde) criterios.push({ tipo: "porfechacargadesde", params: { desde: `${datosFormulario.criterios.porfechacargadesde}T00:00:00` } });
    if (datosFormulario.criterios.porfechacargahasta) criterios.push({ tipo: "porfechacargahasta", params: { hasta: `${datosFormulario.criterios.porfechacargahasta}T23:59:59` } });
    if (datosFormulario.criterios.porfechaacontecimientodesde) criterios.push({ tipo: "porfechaacontecimientodesde", params: { desde: `${datosFormulario.criterios.porfechaacontecimientodesde}T00:00:00` } });
    if (datosFormulario.criterios.porfechaacontecimientohasta) criterios.push({ tipo: "porfechaacontecimientohasta", params: { hasta: `${datosFormulario.criterios.porfechaacontecimientohasta}T23:59:59` } });
  ...
  const seleccionarColeccion = (coleccion) => {
    ...
    setFormData({
      ...
      criterios: {
        categoria: criterios.filter(c => c.tipo === "porcategoria").map(c => c.params.categoriaDeseada),
  ...
  const guardarColeccion = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Ocurrió un error", { id: toastId, description: error.message });
    }
  };
