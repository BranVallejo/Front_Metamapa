...
const EditarHecho = () => {
  ...
  const inputClass =
    "w-full px-4 py-3 bg-white/50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500 transition-all outline-none text-gray-800 dark:text-gray-100 placeholder-gray-400 backdrop-blur-sm";
  const labelClass =
      ...
      .then((data) => {
        if (data?.hechos?.length > 0) {
          ...
          setFormData({
            ...
            categoria: categoriasPermitidas.includes(hecho.categoria)
              ? hecho.categoria
              : "Otro",
    ...
    try {
      ...
      if (nuevosArchivos.length > 0) {
        ...
        nuevosArchivos.forEach((file) =>
          formDataToSend.append("nuevosArchivos", file)
        );
    ...
    <div className="min-h-screen relative transition-colors duration-500 font-sans text-gray-800 dark:text-gray-100 flex justify-center p-4 pt-28 pb-20">
      ...
      <div className="relative z-10 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md border border-white/20 dark:border-gray-700 p-6 md:p-10 rounded-3xl shadow-2xl w-full max-w-4xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 border-b border-gray-200/50 dark:border-white/5 pb-6">
          <div>
            <span className="text-xs font-bold text-yellow-600 dark:text-yellow-400 uppercase tracking-widest mb-1 block">
              Modo Edición
            </span>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="pt-4 border-t border-gray-200/50 dark:border-white/5">
            <label className={labelClass}>
              Ubicación (Modificar en el mapa)
            </label>
